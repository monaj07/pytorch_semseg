...
from networks import AlexNetGAN
...
parser.add_argument('--outf', default='./saved_GAN', help='folder to output images and model checkpoints')
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        if i % 300 == 0 and i>0:
            ...
            vutils.save_image(fake.data,
                    ...
                    normalize=True)
        if i % 3000 == 0 and i>0:
            torch.save(netG, '%s/netG_epoch_%d_%d.pth' % (opt.outf, epoch, i))
            torch.save(netD, '%s/netD_epoch_%d_%d.pth' % (opt.outf, epoch, i))
